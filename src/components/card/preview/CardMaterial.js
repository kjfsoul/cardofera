...
export const CardMaterial = ({ mesh, imageUrl, style, imageScale, imagePosition, }) => {
    const textureLoader = new THREE.TextureLoader();
    const imageTextureRef = useRef(null);
    const styleTextureRef = useRef(null);
    useEffect(() => {
        const material = mesh.material;
        const styleProps = STYLE_MATERIALS[style];
        Object.assign(material, styleProps);
        textureLoader.load(STYLE_TEXTURES[style], (texture) => {
        ...
        textureLoader.load(STYLE_BUMP_MAPS[style], (bumpMap) => {
        ...
        textureLoader.load(STYLE_NORMAL_MAPS[style], (normalMap) => {
    ...
    const updateMaterialTextures = (material) => {
        if (imageTextureRef.current && styleTextureRef.current) {
            const uniforms = {
                ...
                uImagePosition: {
                    value: new THREE.Vector2(imagePosition?.x || 0.5, imagePosition?.y || 0.5),
                },
