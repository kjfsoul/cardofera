...
export const safeSelect = (table: string) => ({
  select: (columns: string) => supabase
    .from(table)
    .select(columns)
    .throwOnError()
})
...
export const queryBuilder = <T extends Record<string, unknown>>(table: string) => ({
  select: (columns: string) => ({
    eq: (column: keyof T, value: unknown) =>
      safeSelect(table).select(columns).eq(column as string, value)
  })
})